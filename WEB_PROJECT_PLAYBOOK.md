...
- `[REQUIRED]` **This applies identically to EFFORT, not just MODEL.** A wrong effort tier (e.g. active tier is low, the task needs medium or higher) is stopped exactly the same way as a wrong model — name the correct tier and wait. Do not silently proceed at the wrong effort while merely mentioning the correct one in passing; that is the same defeat-the-point failure as answering after flagging a model mismatch, and it is explicitly banned here so it cannot quietly slide back in.
...
> `[PERSONAL RULE]` **This founder does not read ASK-each-time approval prompts — they click Allow immediately, every time, without exception.** They said so directly: *"i legit never read the manual allow messages i just always press allow straight away since i dont really understand them so i dont get the point of it."* Take this as fact, not a thing to fix by asking them to read more carefully. It changes what MODE actually means for them:
> - ASK-each-time provides **zero real protection** for this founder specifically — it is extra clicking with no corresponding safety benefit, because the click is not preceded by comprehension. The actual safety was never the click; it is the branch, the commit behind it, and the diff review a human or CI performs after the fact (see Section 9).
> - Because of this, the agent must **actively recommend AUTO-accept, not default to ASK-each-time**, for any task that meets the bounded-auto-edit conditions in Section 9 (clean branch, clear scope, reversible, no secrets/production/external systems). Defaulting to ASK "to be safe" is not actually safer for this founder — it just adds friction while providing the same false comfort clicking-through already gives them.
> - ASK-each-time is still the right call — and should still be recommended — for anything Section 9 already flags for manual approval regardless of mode (unfamiliar scripts, package installs, network access, deletions outside scope, Git publication/history changes, deployment, credentials). Those categories need a real human decision, not a rubber-stamp click, so they stay gated no matter what.
> - This is a `PERSONAL RULE` about this founder's actual behavior, not a claim that ASK-each-time is generally useless — for a human who *does* read prompts, it remains genuinely protective (Section 9's own reasoning for it still holds for everyone else).
...
2. Freeze the approved reference set in `/docs/references/`.
   - If any reference is a live link, the agent attempts to render and screenshot it directly. If it fails after a reasonable retry (reload once, allow extra load time), the agent stops trying and reports the exact URL(s) that failed and why (e.g. "did not finish loading," "crashed the browser tool"), then asks for a screen recording — **restating exactly what the human originally said they liked about that reference**, word for word or close to it, so the human knows precisely which section/moment to capture rather than having to remember or guess. For example: *"I can't render [URL] myself — heavy animated/3D sites sometimes fail in automated browsing even though they work fine in your own browser. You mentioned you liked '[the human's own words]' — could you send a short screen recording of that specific part? I'll inspect it frame by frame."* Never send a generic "send a recording" request without repeating back what the human actually said. The agent then reviews the supplied recording at key moments (not just the first frame) to extract the actual layout, timing, and motion detail — with the same rigor as a working screenshot — before writing it into `DESIGN.md`.
